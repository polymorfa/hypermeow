...
//
// The returned value is a real *Client with the same node dispatcher as
// [NewClient], so registering event handlers, injecting nodes
...
func (cli *Client) InjectNode(ctx context.Context, node *waBinary.Node) error {
	...
	if h := cli.RawNodeHandler; h != nil {
		...
		if drop {
			cli.recvLog.Debugf("RawNodeHandler dropped injected node: %s", node)
			return nil
		}
		if modified != nil {
			node = modified
