...
func (cli *Client) handleDeviceNotification(ctx context.Context, node *waBinary.Node) {
	...
	for _, child := range node.GetChildren() {
		...
		if newParticipantHash == deviceHash {
			...
			putBoundedCache(cli.userDevicesCache, from, cached, maxUserDeviceCacheEntries)
		} else {
		...
		if fromLID != nil && changedDeviceLID != nil && deviceLIDHash != "" {
			...
			if newLIDParticipantHash == deviceLIDHash {
				...
				putBoundedCache(cli.userDevicesCache, *fromLID, cachedLID, maxUserDeviceCacheEntries)
			} else {
...
func (cli *Client) handleOwnDevicesNotification(ctx context.Context, node *waBinary.Node, fromJID types.JID) {
	...
	if newHash != expectedNewHash {
		...
		putBoundedCache(cli.userDevicesCache, fromJIDPlain, deviceCache{devices: newDeviceList, dhash: expectedNewHash}, maxUserDeviceCacheEntries)
		putBoundedCache(cli.userDevicesCache, altJID, deviceCache{devices: altDeviceList, dhash: participantListHashV2(altDeviceList)}, maxUserDeviceCacheEntries)
	}
}
